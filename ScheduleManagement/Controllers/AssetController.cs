    ...
    public class AssetController : Controller
    {
        ...
        public ActionResult CreateSchedule(ScheduleMachine sm)
        {
            ...
            if (!isExist)
            {
                ...
                at.InsrtHistory((tblEmployee)Session["EmployeeData"], sm.Id, "ScheduleMachine", "NA", "Created Asset", sm.Remark);
            ...
            else
            {
                ...
                {
                    at.InsrtHistory((tblEmployee)Session["EmployeeData"], sm.MachineId, "ScheduleMachine", olditem.MachineId.ToString(), sm.MachineId.ToString(), sm.Remark);
                    scheModel.Entry(sm).Property(x => x.MachineId).IsModified = true;
                }
                if (olditem.MachineLocation != sm.MachineLocation)
                {
                    at.InsrtHistory((tblEmployee)Session["EmployeeData"], sm.MachineId, "ScheduleMachine", olditem.MachineLocation.ToString(), sm.MachineLocation.ToString(), sm.Remark);
                    scheModel.Entry(sm).Property(x => x.MachineLocation).IsModified = true;
                }
                if (olditem.ScheduleType != sm.ScheduleType)
                {
                    at.InsrtHistory((tblEmployee)Session["EmployeeData"], sm.MachineId, "ScheduleMachine", olditem.ScheduleType.ToString(), sm.ScheduleType.ToString(), sm.Remark);
                    scheModel.Entry(sm).Property(x => x.ScheduleType).IsModified = true;
                }
                if (olditem.StartScheduleDate != sm.StartScheduleDate)
                {
                    at.InsrtHistory((tblEmployee)Session["EmployeeData"], sm.MachineId, "ScheduleMachine", olditem.StartScheduleDate.ToString(), sm.StartScheduleDate.ToString(), sm.Remark);
                    scheModel.Entry(sm).Property(x => x.StartScheduleDate).IsModified = true;
                }
                if (olditem.EndScheduleDate != sm.EndScheduleDate)
                {
                    at.InsrtHistory((tblEmployee)Session["EmployeeData"], sm.MachineId, "ScheduleMachine", olditem.EndScheduleDate.ToString(), sm.EndScheduleDate.ToString(), sm.Remark);
                    scheModel.Entry(sm).Property(x => x.EndScheduleDate).IsModified = true;
                }
                if (olditem.AsignTo != sm.AsignTo)
                {
                    at.InsrtHistory((tblEmployee)Session["EmployeeData"], sm.MachineId, "ScheduleMachine", olditem.AsignTo.ToString(), sm.AsignTo.ToString(), sm.Remark);
                    scheModel.Entry(sm).Property(x => x.AsignTo).IsModified = true;
                }
                if (olditem.Status != sm.Status)
                {
                    at.InsrtHistory((tblEmployee)Session["EmployeeData"], sm.MachineId, "ScheduleMachine", olditem.Status.ToString(), sm.Status.ToString(), sm.Remark);
                    scheModel.Entry(sm).Property(x => x.Status).IsModified = true;
                }
                if (olditem.AsignType != sm.AsignType)
                {
                    at.InsrtHistory((tblEmployee)Session["EmployeeData"], sm.MachineId, "ScheduleMachine", olditem.AsignType.ToString(), sm.AsignType.ToString(), sm.Remark);
                    scheModel.Entry(sm).Property(x => x.AsignType).IsModified = true;
                }
                if (olditem.ApprovedDate != sm.ApprovedDate)
                {
                    at.InsrtHistory((tblEmployee)Session["EmployeeData"], sm.MachineId, "ScheduleMachine", olditem.ApprovedDate.ToString(), sm.ApprovedDate.ToString(), sm.Remark);
                    scheModel.Entry(sm).Property(x => x.ApprovedDate).IsModified = true;
                }
                if (olditem.CreatedBy != sm.CreatedBy)
                {
                    at.InsrtHistory((tblEmployee)Session["EmployeeData"], sm.MachineId, "ScheduleMachine", olditem.CreatedBy.ToString(), sm.CreatedBy.ToString(), sm.Remark);
                    scheModel.Entry(sm).Property(x => x.CreatedBy).IsModified = true;
                }

                at.InsrtHistory((tblEmployee)Session["EmployeeData"], sm.MachineId, "ScheduleMachine", olditem.CreatedDate.ToString(), DateTime.Now.ToString(), sm.Remark);
                //}
                scheModel.SaveChanges();
            ...
            foreach (var item in lstBndData)
            {
                ...
                foreach (var child in grpchild)
                {
                    if (child.menu_Id == 12)
                    {
                        ...
                        {
                            item.Button = " <button class=\"btn btn-minier btn-white btn-danger btn-bold\" data-toggle=\"modal\" data-target=\"#AddRemark\" onclick=GetID(\"" + Convert.ToString(item.Id) + "\") title=\"Delete Service\" ><i class=\"ace-icon fa fa-trash bigger-80\"></i></button>"
                           + " <button class=\"btn btn-minier btn-white btn-danger btn-bold\" onclick=ScheduleMail(\"" + Convert.ToString(item.Id) + "\") title=\"Send Email\" ><i class=\"ace-icon fa fa-envelope bigger-80\"></i></button>";
